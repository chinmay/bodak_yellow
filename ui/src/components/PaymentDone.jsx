...
class PaymentDone extends React.PureComponent {
  render() {
    const { history, t } = this.props;

    return (
      <div className="done-container">
        <img style={{ marginBottom: '16px' }} src={checkIcon} alt="Runs on Ripple" />
        <p className="text">{t('payment_sent')}</p>

        <button className="button inverse" onClick={() => history.push('/create-payment')}>
          {t('done')}
        </button>
...
export default translate()(withRouter(PaymentDone));
